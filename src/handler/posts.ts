...
export class PostsHandler {
  ...
  public get(req: express.Request, res: express.Response, next: express.NextFunction) {
    this.pool.query(LATEST_POSTS_PAGED, [5, 0]).then(result => {
      return res.json(PostsHandler.toPagedResult(result));
    }).catch(e => {
  ...
  private static toPagedResult(posts: any): PagedResult<Post> {
    let pagedResult: PagedResult<Post> = new PagedResult<Post>();
    for (let i = 0; i < posts.rows.length; i++) {
      pagedResult.data[posts.rows[i].id] = posts.rows[i];
      pagedResult.order.push(posts.rows[i].id);
    }
